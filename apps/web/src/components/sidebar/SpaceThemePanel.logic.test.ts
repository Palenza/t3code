...
describe("le glissé — verdict arc2.mov (ronds écartés, identité inviolable)", () => {
  it("tirer la DOMINANTE tourne l'anneau AVEC elle, sans retard", () => {
    // corr 0,97-1,00, pente ~1, retard 0 image sur chaque segment net —
    // 9545-10574 : 8,6 s, corr 1,00. La poursuite de la veille (retard ~6
    // images à k=0,15) est morte contre cette mesure.
    const points = poserFigure(0.7, 0.5, 3);
    const avant = ecartsAngulaires(points);
    const apres = deplacerFigure(points, 0.35, 0.72);
    const maintenant = ecartsAngulaires(apres);
    maintenant.forEach((v, i) => expect(v).toBeCloseTo(avant[i]!, 4));
    expect(apres[0]!.x).toBeCloseTo(0.35, 6);
    expect(apres[0]!.y).toBeCloseTo(0.72, 6);
  });

  it("tirer un SATELLITE le fait coulisser SUR l'anneau — lui seul bouge", () => {
    // segment 11102-11581 : le tiré fait 392 px, les autres corr 0,04.
    const points = poserFigure(0.7, 0.5, 3);
    const apres = glisserSatellite(points, 2, 0.5, 0.9);
    expect(apres[0]).toEqual(points[0]);
    expect(apres[1]).toEqual(points[1]);
    // il reste au rayon de l'anneau, à l'angle visé
    const r = Math.hypot(apres[2]!.x - 0.5, apres[2]!.y - 0.5);
    expect(r).toBeCloseTo(Math.hypot(points[0]!.x - 0.5, points[0]!.y - 0.5), 6);
    expect(Math.atan2(apres[2]!.y - 0.5, apres[2]!.x - 0.5)).toBeCloseTo(Math.PI / 2, 4);
  });

  it("le satellite coulissé change les écarts — c'est le réglage fin du dégradé", () => {
    const points = poserFigure(0.7, 0.5, 3);
    const avant = ecartsAngulaires(points);
    const apres = ecartsAngulaires(glisserSatellite(points, 1, 0.5, 0.15));
    expect(apres.join()).not.toBe(avant.join());
  });
});
