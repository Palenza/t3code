...
export const RPC_REQUIRED_SCOPES = {
  ...
  // ── NOS méthodes : dictée vocale locale et modèles de voix ──────────────
  //
  // Portées choisies sur ce qu'elles FONT, pas sur leur famille : ce qui
  // observe est en lecture, ce qui mute ou dépense est en opération. Un
  // téléchargement de plusieurs gigaoctets et la suppression d'un modèle sur
  // disque ne sont pas des lectures.
  //
  // Elles sont ici et non dans `ws.ts` parce que l'amont a déplacé la carte
  // (30/07) : ce Record est EXHAUSTIF par le type, donc ajouter un RPC sans
  // lui choisir de portée devient une erreur de compilation au lieu d'une
  // faille en production. C'est mieux que ce qu'on avait — on adopte.
