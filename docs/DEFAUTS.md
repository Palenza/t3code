...
| **compression du contexte**   | seuil 0,6 → cible 0,2, **30 messages protégés** | _hors de notre main_   | Le SDK impose : ~100 % → 1,7 %, **3 messages**. On ne contrôle ni la cible ni les protégés. Notre vrai levier est ailleurs, et il est mesuré : **54 % de la fenêtre part en résultats d'outils**. Serrer le plafond de sortie vaut mieux que régler un seuil qu'on ne possède pas.   |
...
- ~~Le plafond des images.~~ **Écarté par la mesure, le soir même.** J'avais
  ...
  de la session pèsent **32 810 jetons**, soit **2,5 %** de la fenêtre, pas
  54 %. Il n'y a pas de gain là. Le vrai premier poste, ce sont les résultats
  d'outils (54 %) — c'est-à-dire exactement ce que le plafond à 40 000 vient
  de serrer.
