...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Raptor scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Raptor terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Raptor.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Raptor\'s built-in icon locations.',
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in Raptor.",
      })
  ...
  description:
    "Checked-in project configuration for Raptor (t3.json at the repository root). See https://t3.codes for documentation.",
});
